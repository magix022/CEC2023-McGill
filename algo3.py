...
def algo3():
    ...
    prob += pulp.lpSum([pulp.lpSum([df1.loc[df1['Key'] == resource.name.split('^^^')[0], 'UnitPrice'].values[0] * resource for resource in countries[country]])/df2.loc[df2['Country'] == country, 'Target Donation ($CAD)'].values[0] for country in countries]) 
    ...
    with open('frontend/data/algo3output.json', 'w') as outfile:
